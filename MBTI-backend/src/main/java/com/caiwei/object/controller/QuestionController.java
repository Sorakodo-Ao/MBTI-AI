...
import com.caiwei.object.model.dto.question.*;
...
public class QuestionController {
    ...
    public BaseResponse<List<QuestionContentDTO>> aiGenerateQuestion(
        ...
        String result = zhiPuAiManager.SyncStableZhiPuAi
                (GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage);
        System.out.println("ai 生成题目 result:" + result);
        //生成的题目是markdown编辑器类型文本，需要截取出来题目
        int index = result.indexOf("[");
        int end = result.lastIndexOf("]");
        String questionJson = result.substring(index, end + 1);
        ;
        List<QuestionContentDTO> questionContentDTOList = JSONUtil.toList(questionJson, QuestionContentDTO.class);
